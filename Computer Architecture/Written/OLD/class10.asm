...
addit:
	...
	int 21h
	jmp subtr
_adds: 			;Handles the cases where the addition moves into the tens place.
	sub dl,10
	mov dh,dl
	mov dl,"1"
	int 21h
...
subtr:
	...
	sub dl, [numb]
	cmp dl, 127
	ja _neg          ;Jumps to negative handler by checking to see if the signed int has flowed into unsigned integer.
	mov dh,dl
	mov dl,"0"
	int 21h
	mov dl,dh
...
_mults:                 ;Handles double digit products. Uses loop to keep adding to tens place until number is below 10 and then prints out the ramainder in the ones place
	sub dl,10
	add byte [coun],1
	cmp dl,10
	jge _mults
...
_nn:	            ;Handles Non real numbers when trying to divide by zero
	mov dl, 32
	int 21h
	mov dl,"N"
	int 21h
...
section .data
numf db 0 ;First number storage byte
numb db 0 ;Second number storage byte
coun db 0 ;Counter used for tens place checks - Mainly used in the multiplication block because all other operations come under or max out at 10s place.